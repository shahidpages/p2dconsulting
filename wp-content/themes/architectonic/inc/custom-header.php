 ...
 *
 * @uses architectonic_header_style()
 */
function architectonic_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'architectonic_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => '000000',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'architectonic_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'architectonic' ),
		),
	) );
}
add_action( 'after_setup_theme', 'architectonic_custom_header_setup' );

if ( ! function_exists( 'architectonic_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see architectonic_custom_header_setup().
	 */
	function architectonic_header_style() {
		...
		wp_add_inline_style( 'architectonic-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'architectonic_header_style', 10 );